...
def merge(repository: Repository, other_branch: str):
    ...
    if other_branch not in repository.branches:
        print(f"Error: cannot merge branch {other_branch} as it does not exist")
        return
    ...
    if not need_merge:
        ...
        lca = lcas.pop()
        state_dir_old = repository.state_directory / repository.get_commit(lca).state_hash
        state_dir_newA = repository.state_directory / repository.state_hash
        state_dir_newB = repository.state_directory / repository.get_commit(commit_hash_2).state_hash
        ...
        for path in removed_paths_A.intersection(changed_paths_B):
            print(f"Confict as {path} was removed in {repository.head} and modified in {other_branch}")
        ...
        for path in inserted_paths_A.intersection(inserted_paths_B):
            print(f"Conflict as {path} was inserted in both {repository.head} and {other_branch}")

        files_to_write = [] # we only write the files if there are no merge conflicts in any of them
        for path in possibly_mergable:
            #if path not in repository.file_ids[repository.head]: # we aren't tracking it
            #    continue

            #file_id = repository.file_ids[repository.head][path]
