...
class TextOpChangeLine(Operation):
    ...
    def apply_operation_to_file(self, file):
        file.change_line(self.line_number, self.line_contents)
    ...
    def to_string(self):
        return "TextOpChangeLine\t{}\t{}\t{}".format(self.file_name, self.line_number, self.line_contents)

    @staticmethod
    def from_string(operation_string):
        operation = operation_string.split("\t")
        return TextOpChangeLine(operation[1], int(operation[2]), operation[3])