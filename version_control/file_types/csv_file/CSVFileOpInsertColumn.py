...
class CSVFileOpInsertColumn(Operation):
    ...
    def apply_operation_to_file(self, file):
        file.insert_column(self.index, self.value)
    ...
    def to_string(self):
        return "CSVFileInsertColumn\t{}\t{}\t{}".format(self.file_name, self.index, self.value)

    @staticmethod
    def from_string(operation_string):
        operation = operation_string.split("\t")
        return CSVFileOpInsertColumn(operation[1], operation[2], int(operation[3]))